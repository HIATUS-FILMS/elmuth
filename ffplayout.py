...
def ffplayout_connect():
    ...
    def pass_decrypt():
        ...
        plain_text = cipher_suite.decrypt(read_db()[1].encode('utf-8'))
    ...
    def get_new_access_token():
        ...
        # On récupère les identifiants via tes fonctions
        client_id = read_db()[0]
        client_secret = pass_decrypt()
        ...
        if response.status_code != 200:
            raise Exception(f'Échec de l\'authentification ffplayout : {response.text}')

        data = response.json()

        # ffplayout renvoie 'access' et non 'access_token'
        cached_token = data['access']
        # Les tokens durent généralement 45 minutes, on anticipe le rafraîchissement
